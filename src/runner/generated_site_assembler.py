...
class GeneratedSiteAssembler:
    ...
    def get_site_notes(
        self,
        generated_complete_pages: int,
        mixed_fallback_pages: int,
        partial_pages: int,
        missing_pages: int,
    ) -> list[str]:
        if missing_pages > 0:
            return ["Some generated-site pages are missing from the snapshot."]
        if partial_pages > 0:
            return ["Some generated-site pages are only partially assembled."]
        if mixed_fallback_pages > 0:
            return ["Generated outputs are used where available with stub fallback for the rest."]
        if generated_complete_pages > 0:
            return ["Generated-first page outputs assembled into generated site snapshot."]
        return []

    def run(self) -> dict[str, Any]:
        generated_page_output_index_path = Path(self.settings.artifacts_dir) / "generated_page_output_index.json"
        project_package_path = Path(self.settings.artifacts_dir) / "project_package.json"

        self.logger.info("Loading generated page output index from %s", generated_page_output_index_path)
        generated_page_output_index = self.load_json(str(generated_page_output_index_path))
        ...
        generated_complete_pages = sum(
            1 for page in pages if page["assembly_status"] == "generated_complete"
        )
        mixed_fallback_pages = sum(
            1 for page in pages if page["assembly_status"] == "mixed_fallback"
        )
        ...
        site_snapshot = {
            ...
            "pages": pages,
            "summary": {
                "generated_complete_pages": generated_complete_pages,
                "mixed_fallback_pages": mixed_fallback_pages,
                "partial_pages": partial_pages,
                "missing_pages": missing_pages,
                "total_sections": total_sections,
            },
            "notes": self.get_site_notes(
                generated_complete_pages,
                mixed_fallback_pages,
                partial_pages,
                missing_pages,
            ),
        }

        site_dir = ensure_directory(str(Path(self.settings.outputs_dir) / "generated_site"))
        ...
        artifact_path = save_json_artifact(site_output_index, "generated_site_output_index.json")
        self.logger.info("Saved generated site output index to %s", artifact_path)
        return {
            "generated_site_snapshot": site_snapshot,
            "generated_site_output_index": site_output_index,
        }
